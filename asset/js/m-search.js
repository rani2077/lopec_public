...
async function importModuleManager() {
    let modules = await Promise.all([
        ...
        import(`./spec-point.js?${(new Date).getTime()}`),
    ])
    let moduleObj = {
        fetchApi: modules[0],
        specPoint: modules[1],
...
async function mobileSearchHTML() {
    ...
    let value = urlParams.get("headerCharacterName");
    if (value) {
        inputText = value;
        ...
        // 캐릭터 검색
        Modules.specPoint.getCharacterProfile(inputText, function () {
            document.getElementById("sc-info").innerHTML = Modules.specPoint.searchHtml;
